final : previous : with final.haskell.lib;
{
  myDockerImage = final.dockerTools.buildImage {
      ...
      '';
      config = {
          Cmd = [ "${final.haskellPackages.intray-web-server}/bin/intray-web-server serve --admin syd" ];
          WorkingDir = "/www/intray/data";
          Env = [ "PORT=8000" "API_PORT=8001" ];
        };
    };
  haskellPackages = previous.haskellPackages.override
  { 
    overrides = self: super: 
    let typedUuidRepo = final.fetchFromGitHub {
          owner = "NorfairKing";
          repo = "typed-uuid";
       ...
       typedUuidPkg = name: super.callCabal2nix name (typedUuidRepo + "/${name}") {};
       validityPkg = name: super.callCabal2nix name (validityRepo + "/${name}") {};
       intrayPkg = name: disableLibraryProfiling (super.callCabal2nix name (./. + "/${name}") {});
       mergelessPkg = name: super.callCabal2nix name (mergelessRepo + "/${name}") {};
    in
